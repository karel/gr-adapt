...
void lms_filter_ff_impl::set_mu(float mu) {
    if (mu <= 0.0f || mu > 1.0f) {
        throw std::out_of_range("lms_filter_ff_impl::set_mu: Step size must be in range (0, 1]");
    } else {
        d_mu = mu;
    }
}
...
float lms_filter_ff_impl::error(const float& desired, const float& out) { return desired - out; }

void lms_filter_ff_impl::update_tap(float& tap, const float& in) { tap += d_mu * in * d_error; }
