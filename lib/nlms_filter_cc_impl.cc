...
nlms_filter_cc::sptr nlms_filter_cc::make(bool first_input,
                                          ...
                                          bool reset)
{
    return gnuradio::get_initial_sptr(new nlms_filter_cc_impl(
...
void nlms_filter_cc_impl::set_mu(float mu)
{
    if (mu <= 0.0f || mu >= 2.0f) {
        throw std::out_of_range(
            "nlms_filter_cc_impl::set_mu: Step size must be in range (0, 2)");
    } else {
        d_mu = mu;
    }
}
    ...
    for (int i = 0; i < noutput_items; i++) {
        ...
        if (d_adapt && (!d_skip || d_i >= (d_skip + 1))) {
            ...
            volk_32fc_x2_conjugate_dot_prod_32fc(
                &d_power, &filtered_input[j], &filtered_input[j], (unsigned)l);
#endif // ARMADILLO_FOUND
            d_power += d_epsilon;
