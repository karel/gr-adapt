...
iqrd_rls_filter_ff::sptr iqrd_rls_filter_ff::make(int num_taps,
                                                  ...
                                                  bool reset)
{
    return gnuradio::get_initial_sptr(new iqrd_rls_filter_ff_impl(
...
void iqrd_rls_filter_ff_impl::set_delta(float delta)
{
    if (delta <= 0.0f || delta > 300.0f) {
        throw std::out_of_range("iqrd_rls_filter_ff_impl::set_delta: Regularization "
...
void iqrd_rls_filter_ff_impl::set_lambda(float lambda)
{
    if (lambda <= 0.0f || lambda > 1.0f) {
        throw std::out_of_range("iqrd_rls_filter_ff_impl::set_lambda: Forgetting factor "
                                  ...
                                  gr_vector_void_star& output_items)
{
    ...
    for (int i = 0; i < noutput_items; i++) {
        if (d_adapt && (!d_skip || d_i >= (d_skip + 1))) {
            ...
            arma::fmat xaux =
                (1 / sqrtf(d_lambda)) * input_arma.subvec(k, arma::size(d_taps));
