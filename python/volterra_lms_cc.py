...
class volterra_lms_cc(gr.decim_block):
    """
    docstring for block volterra_lms_cc
    """
    def __init__(self, num_taps, mu1, mu2, adapt, bypass, reset):
        gr.decim_block.__init__(self,
            name="volterra_lms_cc",
            in_sig=[np.complex64, np.complex64],
            out_sig=[np.complex64, np.complex64], decim=1)
        self.mu1 = mu1
        self.mu2 = mu2
        self.adapt = adapt
        self.bypass = bypass
        ...
        self.x = np.zeros(self.length, dtype=np.complex64)
        self.w = np.zeros(self.length, dtype=np.complex64)
    ...
    def work(self, input_items, output_items):
        ...
        for i in range(0, len(output_items[0])):
            ...
            if self.adapt:
                self.w += self.mus * error_out[i] * self.x.conj()
